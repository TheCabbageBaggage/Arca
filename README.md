...
- `AGENT_KEY_SECRET`: Secure random string for agent key hashing
- `BOOTSTRAP_ADMIN_USERNAME`: Default admin username (default: admin)
- `BOOTSTRAP_ADMIN_PASSWORD`: Default admin password (default: admin1234)
- `BOOTSTRAP_ADMIN_EMAIL`: Default admin email (default: admin@arca.local)
...
**Common Issues:**
- **Disk space**: If build fails with no space left on device, run `docker system prune -a -f`
- **Version warning**: Warning about `version` field in docker-compose.yml is safe to ignore
...
**Migration Safety:** Arca uses SQLite. Always run migrations from within the container to ensure consistent DB path (`data/sqlite/arca.db`).

### 5. Seed Initial Data (Optional)

```bash
# Seed with bootstrap data (admin, sample project, contacts, etc.)
sqlite3 data/sqlite/arca.db < scripts/seed-bootstrap.sql

# Or use complete seed with more sample data
sqlite3 data/sqlite/arca.db < scripts/seed-complete.sql

# Reset database completely (with backup)
./scripts/reset-db.sh
```
...
- **Default Admin**: `admin@arca.local` / `admin1234`
...
# Expected output shows success for each test endpoint
# and final confirmation that all tests passed
```
